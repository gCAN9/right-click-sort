...
// NFT collections a wallet holds (Ethereum mainnet), with a thumbnail each.
async function addressCollections(address) {
  if (!/^0x[0-9a-f]{40}$/i.test(address || '')) throw new Error('Bad address');
  const host = BLOCKSCOUT_HOSTS.ethereum;
  const out = [];
  let params = new URLSearchParams({ type: 'ERC-721,ERC-1155' });
  for (let page = 0; page < 12; page++) {
    ...
    for (const c of d.items || []) {
      ...
      out.push({
        name: tok.name || shortAddr(contract),
        symbol: tok.symbol || '',
        contract,
...
async function collectionsResponse(address) {
  try {
...
async function collectionItemsResponse(address, contract) {
  if (!/^0x[0-9a-f]{40}$/i.test(address || '') || !/^0x[0-9a-f]{40}$/i.test(contract || '')) {
    return { status: 400, body: { error: 'Bad address or contract' } };
  }
  try {
    const images = await blockscoutInstances(BLOCKSCOUT_HOSTS.ethereum, contract, address, null);
    if (!images.length) {
      return { status: 404, body: { error: 'No items found for this wallet in that collection.' } };
    }
    return { status: 200, body: { images: images.slice(0, 100) } };
  } catch (e) {
...
async function nftsResponse(rawUrl) {
  ...
  for (const attempt of attempts) {
    try {
      ...
      if (images.length) return { status: 200, body: { images: images.slice(0, 100) } };
