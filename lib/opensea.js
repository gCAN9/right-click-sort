...
// names) from their token id. True edition collections (all tokens sharing
// one artwork, no per-id pattern) are left untouched.
function repairStaleMetadata(entries) {
  ...
  if (!best || pattern.get(best) < 3) return entries;
...
async function blockscoutInstances(host, contract, holder, collectionName) {
  ...
  return repairStaleMetadata(out);
}
