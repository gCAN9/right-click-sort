...
const { noteLimit, recentLimits } = require('./alerts');

// Attach recently-seen upstream limit events to a response body so the UI
// can warn users that results may be incomplete.
function withLimits(body) {
  const limits = recentLimits();
  return limits.length ? { ...body, limits } : body;
}
...
async function fetchRetry(url) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try {
      ...
      if (r.ok) return r;
      if (r.status === 429) noteLimit(`${host} rate limit`, '429');
      lastErr = new Error(`${host} responded ${r.status}`);
    } catch (e) {
      lastErr = e;
    }
  }
  noteLimit(`${host} unavailable`, lastErr.message);
  throw lastErr;
}
...
// CIDv0 (Qm…, base58) → CIDv1 (base32) so it can go in a gateway subdomain.
...
function cidV0toV1(qm) {
  ...
  const cid = [0x01, 0x70, ...num.reverse()]; // CIDv1, dag-pb codec + multihash
  let bits = 0;
  let val = 0;
  let out = 'b';
  for (const b of cid) {
...
// Normalize any IPFS reference (ipfs://, or a gateway path URL) to dweb.link's
// SUBDOMAIN form: it serves with CORS and doesn't bot-block non-browser
// fetchers, unlike ipfs.io and path-form dweb.link these days.
function toIpfsSubdomain(u) {
  if (!u) return u;
  let cid = null;
  let rest = '';
  let m = u.match(/^ipfs:\/\/(?:ipfs\/)?([A-Za-z0-9]+)([^?#]*)/);
  if (!m) m = u.match(/^https?:\/\/[^/]+\/ipfs\/([A-Za-z0-9]+)([^?#]*)/);
  if (m) {
    cid = m[1];
    rest = m[2] || '';
  } else {
  ...
  let v1 = null;
  if (/^Qm[1-9A-HJ-NP-Za-km-z]{44}$/.test(cid)) v1 = cidV0toV1(cid);
  else if (/^baf[a-z0-9]+$/i.test(cid)) v1 = cid.toLowerCase();
  if (!v1) return u;
  return `https://${v1}.ipfs.dweb.link${rest || '/'}`;
}

const ipfsToHttp = (u) => (u && u.startsWith('ipfs://') ? toIpfsSubdomain(u) : u);

function instanceToImage(inst, collectionName) {
  const md = inst.metadata || {};
  let url = inst.image_url || ipfsToHttp(md.image) || null;
  if (!url) return null;
  if (!url.startsWith('data:')) url = toIpfsSubdomain(url);
  return { url, name: md.name || `${collectionName || ''} #${inst.id}`.trim(), id: inst.id };
}

// Public RPC per chain, for reading tokenURI when the indexer's metadata is
// stale and no URL pattern can be inferred.
const RPC_HOSTS = {
...
function decodeAbiString(hex) {
  try {
    ...
    const s = b.slice(64, 64 + len).toString('utf8');
...
// tokenURI(id) with uri(id) (ERC-1155) as fallback.
async function rpcTokenUri(rpc, contract, id) {
  let idHex;
  try {
    idHex = BigInt(id).toString(16).padStart(64, '0');
  } catch {
    return null;
  }
  for (const selector of ['c87b56dd', '0e89341c']) {
    try {
      const r = await fetch(rpc, {
        ...
        body: JSON.stringify({
          ...
          id: 1,
          method: 'eth_call',
          params: [{ to: contract, data: '0x' + selector + idHex }, 'latest'],
        }),
      });
      const j = await r.json();
      if (j && j.result && j.result !== '0x') {
        const s = decodeAbiString(j.result);
        if (s) return s;
      }
    } catch {}
  }
...
// tokenURI value → metadata object. Handles data:application/json (base64 or
// url-encoded) and http(s)/ipfs metadata URLs.
async function tokenUriToMetadata(uri) {
  const dataMatch = uri.match(/^data:application\/json(;base64)?,(.*)$/s);
  if (dataMatch) {
    const raw = dataMatch[1]
      ? Buffer.from(dataMatch[2], 'base64').toString('utf8')
      : decodeURIComponent(dataMatch[2]);
    return JSON.parse(raw);
  }
  const url = toIpfsSubdomain(ipfsToHttp(uri));
  if (!/^https?:\/\//.test(url)) return null;
  return fetchJson(url);
}

function metadataImage(md) {
  let img = md.image || md.image_url || null;
  if (!img && md.image_data) {
    // image_data is either raw inline SVG markup or already a data: URI.
    img = md.image_data.startsWith('data:')
      ? md.image_data
      : `data:image/svg+xml;base64,${Buffer.from(md.image_data, 'utf8').toString('base64')}`;
  }
  if (!img) return null;
  if (img.startsWith('data:')) return img;
  return toIpfsSubdomain(ipfsToHttp(img));
}
...
// Blockscout often caches pre-reveal metadata: many tokens share one
// placeholder image while re-indexed ones show their real "…/{tokenId}"
// image. When that pattern is visible, rebuild the stale entries' URLs (and
// names) from their token id. With no pattern to learn from (e.g. on-chain
// data-URI art where every cached image is the same placeholder), fall back
// to reading tokenURI from the chain RPC for the stale tokens. True edition
// collections (one artwork, no pattern, identical names) are left untouched.
async function repairViaRpc(entries, placeholders, chain, contract) {
  const rpc = RPC_HOSTS[chain];
  if (!rpc || !contract) return entries;
  const out = entries.map((e) => ({ ...e }));
  const stale = out.filter((e) => placeholders.has(e.url) && e.id != null);
  await mapPool(stale, 12, async (e) => {
    try {
      const uri = await rpcTokenUri(rpc, contract, e.id);
      if (!uri) return;
      const md = await tokenUriToMetadata(uri);
      if (!md) return;
      const img = metadataImage(md);
      if (!img) return;
      e.url = img;
      if (md.name) e.name = md.name;
    } catch {}
  });
  return out;
}

async function repairStaleMetadata(entries, chain, contract) {
  const counts = new Map();
  for (const e of entries) counts.set(e.url, (counts.get(e.url) || 0) + 1);
  const placeholders = new Set(
    [...counts].filter(([, c]) => c >= 5).map(([u]) => u)
  );
  if (!placeholders.size) return entries;

  const pattern = new Map(); // "prefix|ext" -> count
  for (const e of entries) {
    if (placeholders.has(e.url) || e.id == null) continue;
    const m = e.url.match(new RegExp(`^(.*/)${e.id}(\\.[A-Za-z0-9]+)?$`));
    if (m) {
      const key = `${m[1]}|${m[2] || ''}`;
  ...
  let best = null;
  for (const [k] of pattern) if (!best || pattern.get(k) > pattern.get(best)) best = k;
  if (!best || pattern.get(best) < 3) {
    // No per-id URL pattern to learn from — read tokenURI from the chain.
    return repairViaRpc(entries, placeholders, chain, contract);
  }
  const [prefix, ext] = best.split('|');

  let nameBase = null;
  for (const e of entries) {
    if (placeholders.has(e.url) || e.id == null) continue;
    const idx = (e.name || '').lastIndexOf(String(e.id));
    if (idx > 0) {
      nameBase = e.name.slice(0, idx);
      break;
    }
  }

  return entries.map((e) => {
    if (!placeholders.has(e.url) || e.id == null) return e;
    return { ...e, url: prefix + e.id + ext, name: nameBase ? nameBase + e.id : e.name };
  });
}
...
// OpenSea free-tier API keys are self-serve (POST /auth/keys, no signup) and
// expire after 7 days — mint one on demand and remint when stale. A
// user-provided OPENSEA_API_KEY always wins.
let osKeyCache = { key: null, at: 0 };
async function getOpenSeaKey() {
  if (API_KEY) return API_KEY;
  if (osKeyCache.key && Date.now() - osKeyCache.at < 6 * 24 * 3600 * 1000) return osKeyCache.key;
  try {
    const r = await fetch('https://api.opensea.io/api/v2/auth/keys', { method: 'POST' });
    if (r.ok) {
      const d = await r.json();
      if (d.api_key) {
        osKeyCache = { key: d.api_key, at: Date.now() };
        return d.api_key;
      }
    }
    noteLimit('opensea key mint failed', `status ${r.status}`);
  } catch (e) {
    noteLimit('opensea key mint failed', e.message);
  }
  return null;
}

const isIpfsUrl = (u) => typeof u === 'string' && (u.includes('/ipfs/') || u.includes('.ipfs.'));

// IPFS-hosted images resolve slowly (or not at all) through public gateways.
// OpenSea archived a CDN copy of every image at indexing time — swap those in
// where available. On rate limit or auth failure the remaining entries just
// keep their gateway URLs.
async function enrichWithOpenSeaCdn(entries, chain, contract) {
  if (!contract) return entries;
  const out = entries.map((e) => ({ ...e }));
  const targets = out.filter((e) => e.id != null && isIpfsUrl(e.url));
  if (!targets.length) return out;
  const key = await getOpenSeaKey();
  if (!key) return out;
  let stop = false;
  await mapPool(targets, 8, async (e) => {
    if (stop) return;
    try {
      const r = await fetch(
        `https://api.opensea.io/api/v2/chain/${chain}/contract/${contract}/nfts/${e.id}`,
        { headers: { 'X-API-KEY': key, Accept: 'application/json' } }
      );
      if (r.status === 429 || r.status === 401) {
        stop = true;
        noteLimit(
          r.status === 429 ? 'opensea api rate limit' : 'opensea key rejected',
          `during ${chain}/${contract}`
        );
        if (r.status === 401) osKeyCache = { key: null, at: 0 };
        return;
      }
      if (!r.ok) return;
      const n = (await r.json()).nft || {};
...
// whole collection.
async function blockscoutInstances(chain, contract, holder, collectionName) {
  ...
  const repaired = await repairStaleMetadata(out, chain, contract);
  return enrichWithOpenSeaCdn(repaired, chain, contract);
}
  ...
  if (parsed.slugs.length) {
    for (const slug of parsed.slugs) {
      ...
      for (const addr of addresses) {
        images.push(...(await blockscoutInstances(chain, contract, addr, slug)));
      }
    }
  } else {
    for (const addr of addresses) {
...
async function profileResponse(q) {
  try {
    return { status: 200, body: withLimits({ results: await profileSearch(q) }) };
  } catch (e) {
...
async function collectionsResponse(address, chain) {
  try {
    if (chain) {
      ...
      return { status: 200, body: withLimits({ collections: await chainCollections(chain, host, address) }) };
    }
    return { status: 200, body: withLimits({ collections: await addressCollections(address) }) };
  } catch (e) {
...
async function collectionSearchResponse(q) {
  ...
  return { status: 200, body: withLimits({ results: deduped.slice(0, 8) }) };
}
...
async function collectionAllResponse(contract, chain) {
  ...
  if (!host) return { status: 400, body: { error: `Unsupported chain "${chain}"` } };
  try {
    const images = await blockscoutInstances(chain || 'ethereum', contract, null, null);
    if (!images.length) {
      return { status: 404, body: { error: 'No items found in that collection.' } };
    }
    // Edition collections mint many copies of one artwork — dedupe.
    const seen = new Set();
    ...
    return { status: 200, body: withLimits({ images: unique.slice(0, MAX_ITEMS) }) };
  } catch (e) {
...
async function collectionItemsResponse(address, contract, chain) {
  ...
  if (!host) return { status: 400, body: { error: `Unsupported chain "${chain}"` } };
  try {
    const images = await blockscoutInstances(chain || 'ethereum', contract, address, null);
    if (!images.length) {
      return { status: 404, body: { error: 'No items found for this wallet in that collection.' } };
    }
    return { status: 200, body: withLimits({ images: images.slice(0, MAX_ITEMS) }) };
  } catch (e) {
    return { status: 502, body: { error: `Could not load items: ${e.message}` } };
  }
}
...
async function nftsResponse(rawUrl) {
  ...
  for (const attempt of attempts) {
    try {
      ...
      if (images.length) return { status: 200, body: withLimits({ images: images.slice(0, MAX_ITEMS) }) };
...
async function imgResponse(raw) {
  ...
  try {
    ...
    if (r.status === 429) noteLimit(`image host ${u.hostname} rate limit`, '429');
